...
const Alerts = () => {
  ...
  const removeAllAlertProducts = () => {
    try {
      localStorage.removeItem('idealPriceProducts');
      setAlertProducts([]);
    } catch (error) {
      console.error("Erro ao excluir produtos de alerta do localStorage:", error);
    }
  };

  const removeAlertProduct = (model) => {
    try {
      const storedProducts = localStorage.getItem('idealPriceProducts');
      if (storedProducts) {
        const parsedProducts = JSON.parse(storedProducts);
        const updatedProducts = parsedProducts.filter(prod => prod.Model !== model);
        console.log("Produtos atualizados:", updatedProducts);
        localStorage.setItem('idealPriceProducts', JSON.stringify(updatedProducts));
        setAlertProducts(updatedProducts);
      }
    } catch (error) {
      console.error("Erro ao excluir produto de alerta do localStorage:", error);
    }
  }
    ...
    <>
      <div id="container-fluid container2">
        ...
        <section className="pt-5">
                ...
                <tbody>
                  {alertProducts.length > 0 ? alertProducts.map((prod, i) => (
                    <tr key={i}>
                      ...
                      <td>
                        <button
                          onClick={() => removeAlertProduct(prod.Model)}
                          type="button"
                          className="btn bg-danger"
                        >
                          <FaRegTrashAlt size={20} className="bg-danger text-white" />
          ...
          <button className="btn btn-danger mx-1" onClick={removeAllAlertProducts}>
            Limpar
          </button>
        </section>
      </div>
